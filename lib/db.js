...
export async function deleteItem(id) {
  const adapter = await db();
  // Cascade deletes handle item_categories, item_topics, item_goals
  return adapter.execute("DELETE FROM items WHERE id = ?", [id]);
}
...
const ALLOWED_SORT_FIELDS = new Set(["date", "title", "type"]);
const SORT_MAP = { date: "i.created_at", title: "i.title", type: "i.type" };

export async function listItems({ category, domain, type, search, topic, concept, goal, from, to, sort, page = 1, limit = 20 } = {}) {
  const adapter = await db();
  let where = [];
  let args = [];
  ...
  const sortField = ALLOWED_SORT_FIELDS.has(sort) ? SORT_MAP[sort] : "i.created_at";
